...
(window.PageModules = window.PageModules || {}).ahorros = (() => {
    ...
    function getActiveBankStyle() {
        const selected = document.getElementById('ahorroGraficaEstilo')?.value || state.bankStyle || 'jar';
        return SHAPE_CONFIG[selected] ? selected : 'jar';
    }

    function applyBankShape(style = getActiveBankStyle()) {
        const shapeMap = {
            jar: { visualId: 'visualJar', clipId: 'clipJar' },
            pig: { visualId: 'visualPig', clipId: 'clipPig' },
            chest: { visualId: 'visualChest', clipId: 'clipChest' }
        };
        const active = shapeMap[style] || shapeMap.jar;

        Object.values(shapeMap).forEach(({ visualId }) => {
            const visual = document.getElementById(visualId);
            if (visual) visual.style.display = visualId === active.visualId ? 'block' : 'none';
        });

        const clipPath = `url(#${active.clipId})`;
        document.getElementById('bankFill')?.setAttribute('clip-path', clipPath);
        document.getElementById('bgRect')?.setAttribute('clip-path', clipPath);
        document.getElementById('bankCoins')?.setAttribute('clip-path', clipPath);
    }
    ...
    function setupListeners() {
        ...
        document.getElementById('ahorroGraficaEstilo').addEventListener('change', () => {
            state.bankStyle = getActiveBankStyle();
            document.getElementById('ahorroGraficaEstilo').value = state.bankStyle;
            applyBankShape(state.bankStyle);
            saveState();
            renderAhorros();
        });
    ...
    function activate() {
        const style = SHAPE_CONFIG[state.bankStyle] ? state.bankStyle : 'jar';
        state.bankStyle = style;
        document.getElementById('ahorroGraficaEstilo').value = style;
        document.getElementById('ahorroMetaPct').value = state.bankGoalPct || 100;
        applyBankShape(style);
        renderAhorros();
    }
