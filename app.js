/* ==== MÓDULO DE CONFIRMACIÓN PERSONALIZADO ==== */
const ConfirmModal = (() => {
    let confirmCallback = null;
    
    const show = (message, itemName = '', options = {}) => {
        confirmCallback = options.callback || null;
        const modal = document.getElementById('confirmModal');
        const messageEl = document.getElementById('confirmMessage');
        const itemEl = document.getElementById('confirmItem');
        const okBtn = document.getElementById('btnConfirmOk');
        
        if (messageEl) messageEl.textContent = message;
        
        if (itemName) {
            itemEl.innerHTML = `<strong>${Utils.escapeHTML(itemName)}</strong>`;
        ...
        else okBtn.textContent = 'Eliminar';
...
const FormValidator = (() => {
    ...
    const validateDescription = (input) => {
        if (!input.value.trim()) {
            setError(input, '❌ Descripción requerida');
            return false;
        }
        if (input.value.length > 30) {
            setError(input, '❌ Máximo 30 caracteres');
            return false;
        }
        setSuccess(input);
        return true;
    ...
    const validateValue = (input) => {
        const valor = Utils.parseCurrency(input.value);
        if (!input.value.trim()) {
            setError(input, '❌ Valor requerido');
            return false;
        }
        if (!isFinite(valor) || valor <= 0) {
            setError(input, '❌ Valor inválido (debe ser > 0)');
            return false;
        }
        setSuccess(input);
        return true;
    ...
    const validateCategory = (input) => {
        if (!input.value || input.value === '__custom__') {
            setError(input, '❌ Selecciona o crea una categoría');
            return false;
        }
        setSuccess(input);
        return true;
    ...
    const validateType = (input) => {
        if (!input.value || input.value === '__custom__') {
            setError(input, '❌ Selecciona o crea un tipo');
            return false;
        }
        setSuccess(input);
        return true;
    ...
    const validateQuotas = (input) => {
        const valor = parseInt(input.value || '0', 10);
        if (valor < 2 || valor > 12) {
            setError(input, '❌ Cuotas entre 2 y 12');
            return false;
        }
        setSuccess(input);
        return true;
...
const Utils = (() => {
    ...
    const parseCurrency = (str) => {
        if (!str || typeof str === 'object') return 0;
        const cleaned = str.toString().trim().replace(/\./g, '').replace(/[^0-9]/g, '');
...
function saveGastoHormiga() {
    ...
    const valor = Number(Utils.parseCurrency(valorRaw)) || 0; // Asegurar que es un número
...
function setupEventListeners() {
    ...
    // Ingresos
    document.getElementById('ingresoCategoria').addEventListener('change', () => {
        FormValidator.validateCategory(document.getElementById('ingresoCategoria'));
    });
    document.getElementById('ingresoValor').addEventListener('input', () => {
        FormValidator.validateValue(document.getElementById('ingresoValor'));
    });

    // Gastos
    document.getElementById('gastoDesc').addEventListener('input', () => {
        FormValidator.validateDescription(document.getElementById('gastoDesc'));
    });
    document.getElementById('gastoTipo').addEventListener('change', () => {
        FormValidator.validateType(document.getElementById('gastoTipo'));
    });
    document.getElementById('gastoValor').addEventListener('input', () => {
        FormValidator.validateValue(document.getElementById('gastoValor'));
    });
    document.getElementById('gastoCuotas').addEventListener('input', () => {
        if (document.getElementById('chkGastoCuotas').checked) {
            FormValidator.validateQuotas(document.getElementById('gastoCuotas'));
        }
    });
    ...
    document.getElementById('btnAddIngreso').addEventListener('click', ()=>{
        ...
        FormValidator.clearValidation(document.getElementById('ingresoCategoria'));
        FormValidator.clearValidation(document.getElementById('ingresoValor'));
    ...
    document.getElementById('btnAddGasto').addEventListener('click', ()=>{
        ...
        FormValidator.clearValidation(document.getElementById('gastoDesc'));
        FormValidator.clearValidation(document.getElementById('gastoTipo'));
        FormValidator.clearValidation(document.getElementById('gastoValor'));
        FormValidator.clearValidation(document.getElementById('gastoCuotas'));
...
const OnboardingTour = (() => {
    const STORAGE_KEY = 'programadorGastosOnboardingV1';
    const steps = [
        {
            selector: '.logo',
            title: 'Bienvenido',
            text: 'Este recorrido te muestra las zonas clave para registrar tu dinero y revisar tu salud financiera.'
        },
        {
            selector: '.card.ingresos',
            title: 'Agrega ingresos',
            text: 'Elige una categoria, escribe el valor en COP y usa Agregar ingreso. Tambien puedes crear nuevos conceptos desde el selector.'
        },
        {
            selector: '.resumen-mes',
            title: 'Revisa el mes',
            text: 'Aqui ves saldo, liquidez y gasto porcentual. Estos indicadores se actualizan al agregar ingresos y gastos.'
        },
        {
            selector: '.card.gastos',
            title: 'Registra gastos',
            text: 'Describe el gasto, selecciona su tipo, define el valor y marca cuotas si aplica. El grafico te muestra avance de pagos y distribucion.'
        },
        {
            selector: '.hormiga-section',
            title: 'Gastos hormiga',
            text: 'Usa Gestionar para anotar compras pequenas del dia a dia. Se suman al total de gastos para que nada se pierda.'
        },
        {
            selector: '#btnGuardar',
            title: 'Crea tu proyeccion',
            text: 'Cuando tengas datos base, este boton replica tu presupuesto en 12 meses para consultar resumen, ahorros e inversiones.'
        },
        {
            selector: '.tab[data-tab="resumen"]',
            title: 'Resumen',
            text: 'En esta seccion comparas los meses creados y revisas como evolucionan ingresos, gastos y liquidez.'
        },
        {
            selector: '.tab[data-tab="ahorros"]',
            title: 'Alcancia y ahorros',
            text: 'En Plan de Ahorros puedes configurar metas y ver la alcancia visual llenarse con el ahorro mensual.'
        },
        {
            selector: '.tab[data-tab="inversiones"]',
            title: 'Inversiones',
            text: 'Cuando tengas dinero disponible, registra inversiones y calcula rendimientos desde esta seccion.'
        }
    ];
    ...
    function ensureElements() {
        ...
        document.body.append(backdrop, highlight, popover);
    }
    ...
    function setMobileNavFor(target) {
        const menuToggle = document.getElementById('menuToggle');
        const mainNav = document.getElementById('mainNav');
        if (!menuToggle || !mainNav) return;
        const isMobileMenu = window.getComputedStyle(menuToggle).display !== 'none';
        if (!isMobileMenu) return;
        if (target?.closest('#mainNav')) mainNav.classList.add('active');
        else mainNav.classList.remove('active');
    }

    function getCurrentStep() {
        while (current < steps.length) {
            const candidate = document.querySelector(steps[current].selector);
            setMobileNavFor(candidate);
            const target = visibleTarget(steps[current].selector);
            if (target) return { step: steps[current], target };
            current += 1;
        }
        return null;
    }
    ...
    function render() {
        ...
        window.setTimeout(() => {
            ...
            popover.innerHTML = `
                <div class="onboarding-kicker">Recorrido inicial</div>
                <h3 class="onboarding-title">${Utils.escapeHTML(step.title)}</h3>
                <p class="onboarding-text">${Utils.escapeHTML(step.text)}</p>
                <div class="onboarding-footer">
                    <span class="onboarding-progress">${current + 1} de ${steps.length}</span>
                    <div class="onboarding-actions">
                        <button class="btn ghost" type="button" data-onboarding-skip>Omitir</button>
                        ${current > 0 ? '<button class="btn secondary" type="button" data-onboarding-prev>Atras</button>' : ''}
                        <button class="btn primary" type="button" data-onboarding-next>${current === steps.length - 1 ? 'Finalizar' : 'Siguiente'}</button>
                    </div>
                </div>
            `;
    ...
    function init() {
        document.getElementById('btnStartOnboarding')?.addEventListener('click', () => start(true));
        document.addEventListener('click', (event) => {
            if (!active) return;
            if (event.target.closest('[data-onboarding-next]')) next();
            if (event.target.closest('[data-onboarding-prev]')) previous();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cerrar menú al seleccionar una opción (solo en móviles)
    navTabs.forEach(tab => {
        tab.addEventListener('click', () => {
    ...
    // Cerrar menú al hacer clic en botones dentro del nav (en móviles)
    ...
    // Cerrar menú al cambiar tamaño de ventana (si cambia de móvil a desktop)
    window.addEventListener('resize', () => {
        if (window.getComputedStyle(menuToggle).display === 'none') {
            mainNav.classList.remove('active');
        }
